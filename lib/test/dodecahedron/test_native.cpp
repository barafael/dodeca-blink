...
void test_short_path() {
    ...
    std::cout << p.get_data().first[0] << std::endl;

    TEST_ASSERT(len == 3);
    node_index expected[3] = { 10, 5, 0 };
    for (size_t i = 0; i < len; i++) { TEST_ASSERT(data.first[i] == expected[i]); }
}
...
void test_18_to_10() {
    ...
    node_index expected[4] = { 10, 15, 19, 18 };
...
void test_2_to_19() {
    ...
    node_index expected[5] = { 19, 15, 16, 11, 7 };
...
void test_9_to_17() {
    ...
    node_index expected[4] = { 17, 18, 13, 9 };
...
/* void test_path() {
    ...
    TEST_ASSERT(p.len() == 3);
    for (size_t i = 0; i < 3; i++) { TEST_ASSERT(p.get_data().first[i] == expected[i]); }
}
*/
